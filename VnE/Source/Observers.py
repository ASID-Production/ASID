...
class LabelObserver(aObserver):
    ...
    def __init__(self, facade, scene, wh=None):
        ...
        self._shader_data = self._facade.addDataBufferToPipeline(self._pipeline)
        self._properties_list = ['coord', 'label_color', 'label_size', 'label', 'label_shift']
        self._properties = {'coord': np.array([0, 0, 0], dtype=np.float32),
                            'label_color': np.array([0, 0, 0, 1], dtype=np.float32),
                            'label_size': np.array([6], dtype=np.float32),
                            'label': None,
                            'label_shift': np.array([0, 0, 0], dtype=np.float32)}
        self._point = None

    def indexPoint(self, point):
        for i, p in enumerate(self._points_dict):
            if p['point'] is point:
                return i
    ...
    def add(self, object, *args, **kwargs):
        ...
        if type(object).__name__ is PointsList.__name__:
            ...
            if object.label is None or object.label == '':
                self._points_dict.append({'point': object, 'pos': -1, 'word': object.label})
                return
            else:
                self._points_dict.append({'point': object, 'pos': self._last_pos, 'word': object.label})
                self._last_pos += len(object.label) * 6
                if None in self._points:
                    ind = self._points.index(None)
                    self._points[ind] = object
                else:
                    ind = len(self._points)
                    self._points.append(object)
                coord = object.coord
                word = object.label
                ...
                try:
                    label_shift = object.__getattribute__('label_shift')
                    if label_shift is not None:
                        label_shift = np.array(label_shift, dtype=self._properties['label_shift'].dtype)
                except AttributeError:
                    label_shift = self._properties['label_shift']
                if label_shift is None:
                    label_shift = self._properties['label_shift']
                label_shift = np.array([label_shift for x in range(len(data))], dtype=np.float32).flatten()
                ...
                self._facade.addDataToShaderData(self._shader_data, 4, label_shift)
                self._facade.addDataToShaderData(self._shader_data, 5, np.array([ind for i in range(len(data))], dtype=np.uintc).flatten())
    ...
    def remove(self, object, *args, **kwargs):
        if type(object).__name__ is PointsList.__name__:
            ...
            try:
                ...
                return
            if self._points_dict[ind]['word'] is None:
                self._points_dict.pop(ind)
                return
            if object in self._points:
                self._points[self._points.index(object)] = None
            pos = self._points_dict[ind]['pos']
            for point_ind in range(ind, len(self._points_dict)):
                self._points_dict[point_ind]['pos'] -= len(self._points_dict[ind]['word']) * 6
            del self._textures[int(pos/6):int(pos / 6 + len(self._points_dict[ind]['word']))]
            self._pipeline_inst.textures = self._textures
            self._facade.deleteDataInShaderData(self._shader_data, 0, pos * self._properties['coord'].nbytes, self._properties['coord'].nbytes * len(self._points_dict[ind]['word']) * 6)
            self._facade.deleteDataInShaderData(self._shader_data, 1, pos * 2 * 4, 2 * 4 * len(self._points_dict[ind]['word']) * 6)
            self._facade.deleteDataInShaderData(self._shader_data, 2, pos * 2 * 4, 2 * 4 * len(self._points_dict[ind]['word']) * 6)
            self._facade.deleteDataInShaderData(self._shader_data, 3, pos * 2 * 4, 2 * 4 * len(self._points_dict[ind]['word']) * 6)
            self._facade.deleteDataInShaderData(self._shader_data, 4, pos * self._properties['label_shift'].nbytes, self._properties['label_shift'].nbytes * len(self._points_dict[ind]['word']) * 6)
            self._facade.deleteDataInShaderData(self._shader_data, 5, pos * 4, 4 * len(self._points_dict[ind]['word']) * 6)
            self._last_pos -= len(self._points_dict[ind]['word']) * 6
            self._points_dict.pop(ind)
