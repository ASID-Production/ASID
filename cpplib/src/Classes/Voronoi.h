...
/**
		 * Clip the cell by a plane and add the resulting clipping face.
		 *
		 * This operation removes vertex/edge portions on the plane's negative side,
		 * creates intersection vertices and connecting edges along the plane, and
		 * appends a new face that separates this cell from the specified neighboring
		 * cell (respecting the provided periodic shift).
		 *
		 * @param clipping_plane Plane used to clip the cell; the cell center must lie on the plane's positive side.
		 * @param id_of_another_cell Identifier of the neighboring cell on the opposite side of the new face.
		 * @param another_shiftcode Shift code describing the periodic-image offset of the neighboring cell.
		 */
namespace cpplib::voronoi {
	...
	/**
			 * Clip this cell by a plane and add the resulting clipping face to the cell.
			 *
			 * Classifies existing vertices relative to the plane, removes portions of the
			 * cell on the negative side, creates intersection vertices and edges where
			 * the plane cuts existing edges, and attaches a new face corresponding to the
			 * clipping plane between this cell and the neighboring cell.
			 *
			 * @param clipping_plane Plane used to clip the cell (cell center must lie on the positive side).
			 * @param id_of_another_cell ID of the neighboring cell on the other side of the clipping plane.
			 * @param another_shiftcode Shift code describing the periodic-image offset of the neighboring cell.
			 */
	class Object {
	...
	public:
		 ...
		 * @param i New unique identifier for the object.
		 */
		...
		/**
		 * Get the object's current lifecycle state.
		 * @returns The current State value.
		 */
		 
		/**
		 * Set the object's lifecycle state.
		 * @param s New State value to assign to the object.
		 */
	...
	struct Vertex : public Object {
		...
		/**
 * Construct a Vertex with the given unique identifier and mark it as VALID.
 *
 ...
 */
	...
	struct Edge : public Object {
		...
		/**
		 * Update this edge's lifecycle state based on its connected vertices and faces.
		 *
		 * Sets the edge to INVALID if it does not have exactly two vertices and two faces.
		 * If it has two vertices and two faces, sets the state to:
		 * - VALID when both vertices are VALID or MODIFICATION,
		 * - MODIFICATION when exactly one vertex is MODIFICATION,
		 * - DELETE when both vertices are DELETE.
		 *
		 * @returns The edge's state after the update.
		 */
		...
		/**
		 * Compute the point where this edge's segment intersects the given plane.
		 *
		 * The edge must be in State::MODIFICATION and not parallel to the plane; the
		 * intersection lies strictly between the edge's two vertices (parametric t in (0,1)).
		 *
		 * @param plane Plane to intersect with the edge segment.
		 * @returns Intersection point on the segment between the two vertices.
		 */
		...
		///
		/**
				 * Return the other vertex of this edge given one of its vertices.
				 * @param v Pointer to one of the two vertices of the edge.
				 * @return Pointer to the other vertex. Asserts if `v` is not a vertex of this edge.
				 */
	...
	public:
		...
		/**
		 * Add a vertex at the specified point, reusing an existing vertex if one already exists at the same coordinates.
		 * @param p Coordinates of the vertex to add.
		 * @return Pointer to the existing vertex whose coordinates match `p` (within the file's tolerance) or to the newly created vertex.
		 */
		...
		/**
			 * Clip this cell by the specified plane and create the new face produced by the cut.
			 *
			 * The cell is modified in-place: vertices, edges, and faces are updated or removed
			 * according to their position relative to the clipping plane; if the plane intersects
			 * the cell, new intersection vertices and edges are created and a new Face representing
			 * the clipping polygon is added and linked into the cell's topology.
			 *
			 * Preconditions:
			 * - The cell center must lie on the positive side of `clipping_plane`.
			 *
			 * @param clipping_plane Plane used to cut the cell (defined in the cell's coordinate space).
			 * @param id_of_another_cell Identifier of the adjacent cell on the other side of the plane;
			 *        stored as the new face's neighbor id.
			 * @param another_shiftcode Periodic boundary shiftcode that aligns the neighboring cell;
			 *        stored with the new face to represent periodic offset.
			 */
		...
		/**
		 * Selects the vertex farthest from the cell center (in Cartesian space) and updates any unset vertex distances.
		 *
		 * @param fractocart Fractional-to-Cartesian transformation matrix used to convert vertex positions for distance computation.
		 * @returns Pointer to the vertex whose squared Cartesian distance from the cell center is maximal among vertices not marked DELETE.
		 */
	...
	public:
		...
		///
		/**
		 * Extract all computed cells and leave the diagram empty.
		 *
		 * @returns Vector of Voronoi cells previously owned by the diagram; the diagram's internal storage will be empty after the call.
		 */
	...
	private:
		...
		/**
		 * Compute and sort neighbor lists by squared Cartesian distance for each flagged cell.
		 *
		 * For each index i where flags_[i] is true, computes the squared Cartesian distance
		 * between point i and each listed neighbor using the fractional-to-Cartesian transform
		 * FtoC applied to (points_in_unit01[i] - points_in_unit01[neighbor] - shift),
		 * stores that squared distance into the third element of each tuple in vec[i],
		 * and then sorts vec[i] in ascending order by that distance. Cells with flags_[i] == false
		 * are left unchanged.
		 *
		 * @param vec Per-cell neighbor lists; each entry is a tuple (neighborIndex, shiftCode, lengthSq).
		 *            The third element is overwritten with the computed squared distance and the
		 *            vector is sorted in-place by that value.
		 * @param points_in_unit01 Fractional coordinates of all points (unit interval).
		 * @param FtoC Matrix converting fractional coordinates to Cartesian coordinates.
		 */
		...
		/**
			 * Clip a Voronoi cell against neighbor bisector planes until no neighbor can alter it.
			 *
			 * Iterates the sorted neighbor list and for each neighbor constructs the bisector plane
			 * (accounting for periodic shift) to clip `cell`. Updates vertex distances as needed
			 * and invokes `cell.clipByPlaneAndAddNewFace` to apply each clipping. The loop performs
			 * an early exit when a neighbor's squared distance is greater than four times the
			 * current farthest vertex squared distance, since such neighbors cannot affect the cell.
			 *
			 * @param cell Voronoi cell to be clipped and modified.
			 * @param vec Sorted list of neighbors; each entry is a tuple (neighbor_index, shift_code, squared_distance).
			 * @param points_in_unit01 Fractional coordinates of all points in the system.
			 * @param unitcell Unit cell used to convert between fractional and Cartesian coordinates.
			 */
	...
	public:
		...
		/**
		 * Associate an edge ID with every polyhedron that contains both specified vertices.
		 *
		 * For each polyhedron index present in both sort_entries[a].cIdx and sort_entries[b].cIdx,
		 * appends `edge` to that polyhedron's `edge_ids`.
		 *
		 * @param a Index of the first vertex in `sort_entries`.
		 * @param b Index of the second vertex in `sort_entries`.
		 * @param edge Edge identifier to add to matching polyhedra.
		 * @param sort_entries Array of SortEntry objects whose `cIdx` lists contain polyhedron indices.
		 */
	...
	private:
		...
		/**
 * Compute the polygon area in Cartesian space using the polygon's vertex ordering.
 *
 * The polygon vertices are transformed from fractional to Cartesian coordinates using FtoC
 * after subtracting the polygon centroid; the area is computed from the projected polygon.
 *
 * @param p Polygon input with vertex indices in consistent cyclic order.
 * @param FtoC Fractional-to-Cartesian transformation matrix.
 * @returns The polygon area in Cartesian units.
 */

/**
 * Compute the solid angle subtended by a polygon at its owning polyhedron center.
 *
 * Vertices are transformed to Cartesian coordinates relative to the polyhedron center
 * and must be provided in consistent cyclic order.
 *
 * @param p Polygon input whose atom_ids[0] identifies the owning polyhedron.
 * @param FtoC Fractional-to-Cartesian transformation matrix.
 * @returns The solid angle in radians (twice the summed triangulation angles).
 */

/**
 * Compute the volume of a polyhedron by summing contributions from its polygons.
 *
 * For each polygon in the polyhedron, the polygon area (in Cartesian units) and the
 * perpendicular distance from the polygon plane to the polyhedron center are used to
 * accumulate a pyramidal/prism-like volume contribution.
 *
 * @param p Polyhedron whose poly_ids reference polygons with precomputed areas.
 * @param FtoC Fractional-to-Cartesian transformation matrix.
 * @returns The polyhedron volume in Cartesian units.
 */
