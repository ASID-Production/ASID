...
namespace cpplib {
	...
	public:
		...
		///
		/**
		 * Check whether two BondEx instances refer to the same atom pair, ignoring the length.
		 *
		 * @param other BondEx instance to compare.
		 * @returns `true` if both objects have identical atom indices (`first` and `second`), `false` otherwise.
		 */
	...
	public:
		...
		/**
 * Shift vector that positions the second atom relative to the first.
 *
 * Represents the offset (for example a periodic-boundary image shift) to apply
 * to the second atom so the bond refers to the correct spatial image.
 */
		PointType shift{};
		...
		///
		/**
		 * Compare two BondWithPoint objects for equality using only their atom indices.
		 *
		 * @param other BondWithPoint to compare against; the point/shift member is ignored.
