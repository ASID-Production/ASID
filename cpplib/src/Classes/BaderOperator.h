...
namespace cpplib {
	...
	public:
		...
		value_type solve_rfo_shift_trust(
			...
			bool maximize,
			value_type R)  const // trust radius
		{
			if (indices.empty()) return value_type(0);
			std::vector<value_type> lambdas, fs;
			for (int idx:indices) {
				lambdas.push_back(lam[idx]); fs.push_back(f[idx]);
			}

			const value_type tiny = 1e-12;
			value_type lambda_bound = maximize?*std::max_element(lambdas.begin(), lambdas.end())
			...
			if (maximize) {
				...
				for (auto fi : fs) f_max_abs = std::max(f_max_abs, std::abs(fi));
			...
			auto Phi = [&](value_type eta) {
				...
				for (size_t i = 0; i < lambdas.size(); ++i) {
					value_type diff = lambdas[i] - eta;
					sum += (fs[i] * fs[i]) / (diff * diff);
				}
				return sum;
				};
		...
		PointType PartitionedRFO() const {
			...
			std::array<value_type, 3> lam = {eig.values[2], eig.values[1], eig.values[0]};
			std::array<PointType, 3> vecs;
			vecs[0] = PointType(eig.vectors[0][2], eig.vectors[1][2], eig.vectors[2][2]); 
			vecs[1] = PointType(eig.vectors[0][1], eig.vectors[1][1], eig.vectors[2][1]); 
			vecs[2] = PointType(eig.vectors[0][0], eig.vectors[1][0], eig.vectors[2][0]); 
			...
			constexpr int max_modes = Target;
			constexpr int min_modes = 3 - max_modes;

			std::vector<int> max_indices, min_indices;

			if constexpr (max_modes > 0) {
				if (!mode_following_init_) {
					...
					std::array<int, 3> assignment;
					assignment.fill(-1);
					std::array<bool, 3> used_old = {false, false, false};

					for (int i_new = 0; i_new < 3; ++i_new) {
						value_type best_overlap = -1.0;
						int best_old = -1;
						for (int j_old = 0; j_old < max_modes; ++j_old) {
							if (used_old[j_old]) continue;
							value_type ov = std::abs(vecs[i_new][0] * prev_max_vecs_[j_old][0]
						...
						if (best_old >= 0 && best_overlap > 0.5) {
							assignment[i_new] = best_old;
					...
					for (int i = 0; i < 3; ++i) {
						if (assignment[i] != -1) {
							max_indices.push_back(i);
							prev_max_vecs_[assignment[i]] = vecs[i]; 
						}
					}

					while (max_indices.size() < static_cast<size_t>(max_modes)) {
						int best_i = -1;
						value_type min_lam = std::numeric_limits<value_type>::max();
						for (int i = 0; i < 3; ++i) {
							if (std::find(max_indices.begin(), max_indices.end(), i) == max_indices.end() &&
								std::find(min_indices.begin(), min_indices.end(), i) == min_indices.end()) {
								if (lam[i] < min_lam) {
									min_lam = lam[i]; best_i = i;
								}
							}
						}
						if (best_i >= 0) {
							max_indices.push_back(best_i);
							prev_max_vecs_[max_indices.size() - 1] = vecs[best_i];
						} else break;
					}
				}
			}
			...
			value_type eta_max = 0, eta_min = 0;
			if constexpr (max_modes > 0) {
				eta_max = solve_rfo_shift_trust(max_indices, lam, f, /*maximize=*/true, MAX_STEP);
			}
			if constexpr (min_modes > 0) {
				eta_min = solve_rfo_shift_trust(min_indices, lam, f, /*maximize=*/false, MAX_STEP);
			}

			value_type adaptive_max = MAX_STEP_SQ;
			PointType dx (0, 0, 0);
			for (int i = 0; i < 3; ++i) {
				value_type eta = (i < max_modes)?eta_max:eta_min;
				value_type denom = lam[i] - eta;
				if (std::abs(denom) < 1e-12) denom = (denom >= 0)?1e-12:-1e-12;
				value_type c = -f[i] / denom;
				dx[0] += c * vecs[i][0];
				dx[1] += c * vecs[i][1];
				dx[2] += c * vecs[i][2];
				adaptive_max = std::min(adaptive_max, MAX_STEP_SQ / (c * c));
			}

			const value_type len_sq = dx[0] * dx[0] + dx[1] * dx[1] + dx[2] * dx[2];
			if (len_sq > adaptive_max) {
				const value_type scale = std::sqrt(adaptive_max / len_sq);
				dx[0] *= scale;
				dx[1] *= scale;
				dx[2] *= scale;
			}
	...
	public:
		...
		static inline void UpdateType(CriticalPoint& cp) {
			auto eigen = cp.value_.hess.EigenvaluesAndVectors();
			int rank = static_cast<int>(eigen.values[0] < 0) +
				static_cast<int>(eigen.values[1] < 0) +
				static_cast<int>(eigen.values[2] < 0);
			//cp.type_ = static_cast<CriticalPoint::TYPE>(rank);
		}

		static void OptimizePoint(CriticalPoint& cp, const PointsSoA<value_type>& psoa, value_type end_sq) {
			cp.UpdateVal(CriticalPoint::CalculateEDinPoint(cp.pos_, psoa, ElectronDensitySplines));
			value_type drs = 1;
			do {
				PointType next_point = cp.FindNextPosition();
				//next_point = AdaptiveOptimizePoint(next_point, cp, psoa);
		...
		static PointType AdaptiveOptimizePoint(PointType next, const CriticalPoint& cp, const PointsSoA<value_type>& psoa) {
			auto next_val = CriticalPoint::CalculateEDinPoint(next, psoa, ElectronDensitySplines);
			auto gSq = cp.value_.getGradSq();
			auto g_nextSq = next_val.getGradSq();
			while (gSq <= g_nextSq) {
				next += cp.pos_;
				next *= 0.5;
				next_val = CriticalPoint::CalculateEDinPoint(next, psoa, ElectronDensitySplines);
				g_nextSq = next_val.getGradSq();
			}
			return next;
		}
